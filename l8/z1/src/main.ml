...
let rec cp_env env e = 
  match e with
  | Int n -> Int n
  | Bool b -> Bool b
  | If (p, t, e) ->
        match cp_env env p with
        | Bool true -> cp_env env t 
        | Bool false -> cp_env env e
        | _ -> e
  | Binop (op, e1, e2) -> 
        match (cp_env env e1, cp_env env e2) with
        | (Int v1, Int v2) -> Int eval_op op (VInt v1) (VInt e2)
        | (Int v1, Bool v2) -> 
        | (Bool v1, Int v2) -> 
        | (Bool v1, Bool v2) -> 
        | _ ->
  | Binop (op, e1, e2) -> eval_op op (cp_env env e1) (cp_env env e2)
  | Let (x, e1, e2) ->
      let r = eval_env env e1 in
      let new_env = M.update x (fun _ -> Some r) env in
      eval_env new_env e2
