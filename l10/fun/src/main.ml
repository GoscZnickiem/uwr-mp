...
let rec eval_env (env : env) (e : expr) : value =
  ...
  | Ref e -> 
    ...
    let loc = fresh_location () in  (* Funkcja fresh_location tworzy nowy unikalny identyfikator komórki pamięci *)
    let new_env = M.add loc v env in
    loc
  | Deref e -> 
    (match eval_env env e with
    | VLocation loc -> (match M.find_opt loc env with
        | Some v -> v
        | None -> failwith "unbound location")
    | _ -> failwith "type error")
  | Assign (e1, e2, e3) -> 
    (match eval_env env e1 with
    | VLocation loc -> 
        let v2 = eval_env env e2 in
        let new_env = M.add loc v2 env in
        eval_env new_env e3
    | _ -> failwith "type error")
  | App (e1, e2) ->
